 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i=0; i<N-1; i++) {
      ...
      fg[0] += 9*CppAD::pow(vars[a_start+i], 2);
    }
    ...
    for (int i=0; i<N-1; i++) {
      ...
      // Actuations at time t - i.e. between t and t+1
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // setting number of variables - 6 element state vector + 2 element actuation vector
  ...
  // set all non-actuators upper/lower limits to +/- infinity
  for(int i=0; i<delta_start; i++) {
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] = 1.0e19;
  }

  // set upper/lower limit of delta to -25 to 25 degrees - values in radians
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }

  // set upper.lower limit of the throttle
